...
export function useAccountSearch(query, options = {}) {
    ...
    const results = useMemo(() => {
        ...
        const matchingAccounts = mockAccounts
            .filter((account) => account.username.toLowerCase().startsWith(trimmedQuery))
            .slice(0, maxResults);
        // Add existing accounts from mock data
        accountResults.push(...matchingAccounts.map((account) => ({
        ...
        const exactMatch = accountResults.find((result) => result.username.toLowerCase() === trimmedQuery.toLowerCase());
...
//# sourceMappingURL=useAccountSearch.mock.js.map