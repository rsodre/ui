...
export const StarryHeaderBackground = ({ height = 136, backgroundColor = "#171A17", starColor = "#FBCB4A", className, reducedMotion = false, }) => {
    ...
    const createStarInLayer = useCallback((starfield, stars, layerClass, minSize, maxSize, minOpacity, maxOpacity, containerWidth, containerHeight, returnData = false, forceX = null, forceY = null) => {
    ...
    const createAllStars = useCallback(() => {
        ...
        for (let i = 0; i < FOREGROUND_STARS; i++) {
            createStarInLayer(starfield, containerStars, "foreground", 2.0, 3.5, 0.7, 1.0, containerWidth, containerHeight);
        }
        // Create middle stars (original count)
        for (let i = 0; i < MIDDLE_STARS; i++) {
            createStarInLayer(starfield, containerStars, "middle", 1.5, 2.8, 0.5, 0.8, containerWidth, containerHeight);
        }
        ...
        while (currentBackgroundStars < BACKGROUND_STARS) {
            const primaryStarData = createStarInLayer(starfield, containerStars, "background", 0.5, 1.3, 0.3, 0.5, containerWidth, containerHeight, true);
            if (!primaryStarData)
                continue;
            currentBackgroundStars++;
            const potentialTotalAfterCluster = currentBackgroundStars + STARS_PER_CLUSTER;
            if (Math.random() < CLUSTER_CHANCE &&
                ...
                for (let j = 0; j < STARS_PER_CLUSTER; j++) {
                    ...
                    createStarInLayer(starfield, containerStars, "background", 0.4, 1.0, 0.25, 0.45, containerWidth, containerHeight, false, clusterX, clusterY);
                    currentBackgroundStars++;
                }
            }
            else {
    ...
    const animate = useCallback((currentTime) => {
        ...
        for (let i = 0; i < allStars.length; i++) {
            ...
            if (!reducedMotion) {
                ...
                newOpacity = Math.max(star.initialOpacity * star.minOpacityFactor, Math.min(star.initialOpacity * star.maxOpacityFactor, star.initialOpacity + twinkleAmount));
            }
    ...
    return (_jsxs("div", { ref: containerRef, className: cn("starry-container", "relative overflow-hidden", className), style: {
            width: "100%",
            height: `${height}px`,
            backgroundColor: backgroundColor,
        }, children: [_jsx("div", { ref: starfieldRef, className: "starfield absolute inset-0" }), _jsx("div", { className: "gradient-fade" })] }));
};
//# sourceMappingURL=starry-header.js.map